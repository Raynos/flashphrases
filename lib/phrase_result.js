...
CachedObject.defineProperty(PhraseResult.prototype, 'got', function() {
    var session = this.session;
    if (session.input && session.input.length) {
        return session.input[session.input.length-1].got;
    }
});
